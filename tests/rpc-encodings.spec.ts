...
describe("rpc", () => {
  ...
  it("protobyheight", async () => {
    expect(encodeProtorunesByHeightInput(849235, BigInt(1))).to.equal(
      "0x08d3ea3312020801"
    );
  });
  ...
  it("alkanes outpoint by id encoding", async () => {
    expect(encodeAlkanesIdToOutpointInput(BigInt(2), BigInt(16))).to.equal(
      "0x0a080a02080212020810"
    );
  });
