...
function encodeRunestoneProtostone(runestone) {
    const mint = runestone.mint
        ? (0, monads_1.Some)(new runeid_1.RuneId(BigInt(u64Strict(BigInt(runestone.mint.block))), BigInt(u32Strict(BigInt(runestone.mint.tx)))))
        : monads_1.None;
    ...
    const edicts = (runestone.edicts ?? []).map((edict) => ({
        id: new runeid_1.RuneId(u64Strict(BigInt(edict.id.block)), u32Strict(BigInt(edict.id.tx))),
