...
class RunestoneProtostoneUpgrade {
    ...
    encipher() {
        ...
        if (this.protostones.length) {
            ...
            const packed = all_protostone_payloads.reduce((r, v) => Buffer.from(Array.from(r).concat(Array.from((0, bytes_1.encodeVarInt)((0, bytes_1.fromBuffer)(v))))), Buffer.from([]));
            const u128s = (0, lodash_1.chunk)(Array.from(packed), exports.MAX_U128_BYTES_COMPAT_W_RUNES).map((v) => (0, integer_1.u128)(BigInt("0x" + Buffer.from(v).toString("hex"))));
